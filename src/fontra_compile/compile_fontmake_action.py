...
class CompileFontMakeAction:
    ...
    @asynccontextmanager
    async def connect(self, input: ReadableFontBackend) -> AsyncGenerator["CompileFontMakeAction", None]:
        self.input = input
        try:
            yield self
        finally:
    ...
    async def process(self, outputDir: pathlib.Path = pathlib.Path(), *, continueOnError=False) -> None:
        ...
        fontmake_main([
            "-m" if source_path.suffix == ".designspace" else "-u", str(source_path),
            "-o", "variable" if is_variable else "ttf",
            "--output-path", str(outputFontPath),
